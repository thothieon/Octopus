...
class GoogleSheet():
    ...
    def parse04course(self, doc_key, name, sheet):
        ...
        #月份
        monthvalue = "01/"
        filt = self.df[0].str.contains(yearvalue, na=False) & ( (self.df[4].str.contains(monthvalue) & (self.df[4].str.contains('定金', na=False) | self.df[4].str.contains('全額', na=False))) | (self.df[5].str.contains(monthvalue) & (self.df[5].str.contains('定金', na=False) | self.df[5].str.contains('全額', na=False))) )
    ...
    def parse04CourseDayTrading(self, doc_key, name, sheet, tradingdata):
        ...
        filt = self.df[0].str.contains(yearvalue, na=False) & ( (self.df[4].str.contains(tradingdata) & (self.df[4].str.contains('訂金', na=False) | self.df[4].str.contains('定金', na=False) | self.df[4].str.contains('全額', na=False))) | (self.df[5].str.contains(tradingdata) & (self.df[5].str.contains('訂金', na=False) | self.df[5].str.contains('定金', na=False) | self.df[5].str.contains('全額', na=False))) )
        #print(self.df.loc[filt, [0, 1, 4, 5, 6]])
        #sdff = self.df.loc[filt, [0, 1, 4, 5, 6]]
    ...
    def parse04coursesearchpaid(self, doc_key, name, sheet):
        ...
        #月份
        monthvalue = "1/"
        filt = self.df[0].str.contains(yearvalue, na=False) & ( (df[4].str.contains(monthvalue) & (df[4].str.contains('定金', na=False) | df[4].str.contains('全額', na=False))) | (df[5].str.contains(monthvalue) & (df[5].str.contains('定金', na=False) | df[5].str.contains('全額', na=False))) )
        print(self.df.loc[filt, [0, 1, 4, 5, 6]])
    ...
    def parseSaveXlsx(self, doc_key, name, sheet):
        ...
        postscolnamexlsx = "postscol_" + name + "_" + datetime_dt.strftime("%Y%m%d%H%M%S") + ".xlsx"
        ...
        for i in range(sheet):
            ...
            df.to_excel(writer, sheet_name=worksheetname, index=False, encoding="utf_8_sig")
